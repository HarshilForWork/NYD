...
class ShlokExtractor:
    ...
    def __init__(self):
        ...
        # This pattern accounts for spaces around dashes and letters
    ...
    def extract_shloks(self, text: str) -> List[Dict[str, Any]]:
        ...
        for book_section in book_sections:
            ...
            for sarga_section in sarga_sections:
                ...
                # Extract all shloks in this sarga with improved handling
    ...
    def extract_shloks_from_sarga(self, sarga_text: str, book: str, sarga: str) -> List[Dict[str, Any]]:
        """
        Extract shloks from a single sarga with improved numbering handling.
        
        Args:
            sarga_text: Text of the sarga
            book: Book name
            sarga: Sarga number
            
        Returns:
            List of shlok dictionaries
        """
        shloks = []
        ...
        for i, match in enumerate(shlok_matches):
            shlok_num = match.group(1).strip()  # This will preserve "12- 13a", "13b- 14a" with spaces
            shlok_text = match.group(2).strip()
            
            # Clean up the shlok text - remove excessive whitespace but preserve the shlok number exactly
            shlok_text = re.sub(r'\s+', ' ', shlok_text)
            
            # Skip if shlok text is too short (likely parsing error)
            if len(shlok_text.strip()) < 10:
                continue
            
            # Create individual document for this specific shlok number
            # Important: shlok_num preserves exact formatting including spaces like "12- 13a"
            shlok_data = {
                "book": book,
                "sarga": sarga,
                "shlok": shlok_num,  # This will preserve exact numbering like "12- 13a", "13b- 14a"
                "text": shlok_text,
            ...
            # Debug print to verify correct extraction
            if st.session_state.get('debug_mode', False):
                st.write(f"Extracted: Book={book}, Sarga={sarga}, Shlok={shlok_num}")
...
def load_db():
    ...
    for i, shlok in enumerate(shloks):
        ...
        # The shlok number will be preserved exactly as it appears (e.g., "12- 13a", "13b- 14a")
...
Shlok {shlok['shlok']}: {shlok['text']}"""
        ...
        metadata = {
            "book": shlok['book'],
            "sarga": shlok['sarga'],
            "shlok": shlok['shlok'],  # Exact shlok number preserved with spaces like "12- 13a"
            "doc_id": i
        }
...
def main():
    ...
    # Add a section to show database statistics if in debug mode
    if st.session_state.get('debug_mode', False) and 'db' in st.session_state and st.session_state.db:
        with st.expander("Database Statistics"):
            # Get a sample of documents to show shlok numbering
            sample_docs = st.session_state.db.similarity_search("sample", k=10)
            
            st.write("Sample of shlok numbers in database:")
            shlok_numbers = []
            for doc in sample_docs:
                if 'shlok' in doc.metadata:
                    shlok_numbers.append(doc.metadata['shlok'])
            
            # Show unique shlok number formats
            unique_shloks = list(set(shlok_numbers))[:20]  # Show first 20 unique ones
            for shlok in sorted(unique_shloks):
                st.write(f"- {shlok}")
            
            st.write(f"Total documents in database: {st.session_state.db.index.ntotal}")
