...
class ShlokExtractor:
    ...
    def __init__(self):
        ...
        self.book_pattern = re.compile(r'(BALA|AYODHYA|ARANYA|KISHKINDA|SUNDARA|YUDDHA)\s+KANDA', re.IGNORECASE)
        self.sarga_pattern = re.compile(r'SARGA\s+(\d+)', re.IGNORECASE)
        self.shlok_pattern = re.compile(r'Shlok\s+(\d+):(.*?)(?=Shlok\s+\d+:|$)', re.DOTALL)
...
Shlok {shlok['shlok']}: {shlok['text']}"""
    ...
    # Add some larger context documents as well (grouping several adjacent shloks)
    context_docs = []
    for i in range(0, len(shloks), 5):  # Group shloks in chunks of 5
        group = shloks[i:i+5]
        if not group:
            continue
            
        # Combine 5 shloks into one document for better context
        combined_text = f"""Book: {group[0]['book']} KANDA
Sarga: {group[0]['sarga']}
"""
        for shlok in group:
            combined_text += f"Shlok {shlok['shlok']}: {shlok['text']}\n\n"
            
        metadata = {
            "book": group[0]['book'],
            "sarga": group[0]['sarga'],
            "shlok_range": f"{group[0]['shlok']}-{group[-1]['shlok']}",
            "is_context": True,
            "doc_id": f"context_{i//5}"
        }
        
        context_docs.append(Document(page_content=combined_text, metadata=metadata))
    
    # Combine individual shlok docs and context docs
    all_docs = docs + context_docs
...
def verify_statement(statement, db):
    ...
    relevant_docs = db.similarity_search(statement, similarity_threshold=0.9)
