...
def main():
    ...
    """
    print("=== Ramayana Text Parser - Multiple Files Support ===\n")
    ...
    if 'Kanda/Book' in df.columns and 'Sarga/Chapter' in df.columns:
        ...
        df = df.sort_values(['sort_order', 'Sarga/Chapter', 'Shloka/Verse Number']).drop('sort_order', axis=1)
    ...
    try:
        ...
        if 'Kanda/Book' in df.columns:
            print(f"\nBreakdown by Kanda:")
            book_counts = df.groupby('Kanda/Book').size().sort_values(ascending=False)
        ...
        print(f"  - 'Verses': All combined data")
...
def create_summary_sheet(df):
    ...
    summary = []
    if 'Kanda/Book' in df.columns:
        kanda_counts = df['Kanda/Book'].value_counts().sort_index()
        for kanda, count in kanda_counts.items():
            summary.append({'Kanda/Book': kanda, 'Total Verses': count})
    total_verses = len(df)
