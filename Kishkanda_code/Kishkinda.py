...
def convert_csv_to_txt(input_file, output_file):
    ...
    df = df.sort_values(['Kanda/Book', 'Sarga/Chapter', 'Shloka/Verse Number'], 
                       key=lambda x: pd.to_numeric(x, errors='coerce') if x.name == 'Shloka/Verse Number' else x)
    
    # Open output file for writing
    with open(output_file, 'w', encoding='utf-8') as f:
        current_book = None
        current_chapter = None
        verse_counter = 1  # Sequential counter for verses within each chapter
        ...
        for _, row in df.iterrows():
            ...
            if chapter != current_chapter:
                ...
                verse_counter = 1  # Reset verse counter for new chapter
            
            # Write sequential verse number and translation
            f.write(f'{verse_counter}. {translation}\n')
            verse_counter += 1
...
if __name__ == "__main__":
    ...
    convert_csv_to_txt('NYD/Datasets/Kishkinda_kanda_final.csv', 'output.txt')
